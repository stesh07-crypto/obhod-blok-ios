...
const (
	...
	// below that observed kill zone and make GC react earlier during the 108
	// session ramp. This is a soft runtime policy only: it does not alter worker
	// count, TURN/DTLS, WireGuard, packet routing, reconnects, or wire format.
...
func init() {
	...
	// the default during the steep 108-worker startup ramp observed on-device.
